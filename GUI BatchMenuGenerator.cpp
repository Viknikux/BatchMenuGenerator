...
#define SCROLL_CLASS_NAME       L"ScrollContainerClass"
...
// Reposition all child controls based on current scrollPos and container width
static void RepositionChildControls() {
    if (!g.hScrollContainer || g.optionCount <= 0) return;
    RECT rc;
    GetClientRect(g.hScrollContainer, &rc);
    int cx = max(16, rc.right - rc.left);
    int GH = 98, GS = 6;

    for (int i = 0; i < g.optionCount; i++) {
        int by = i * (GH + GS) - g.scrollPos;
        HWND hw;

        hw = GetDlgItem(g.hScrollContainer, IDC_OPTION_GROUP_FIRST + i);
        if (hw) SetWindowPos(hw, NULL, 8, by, cx - 16, GH, SWP_NOZORDER);

        hw = GetDlgItem(g.hScrollContainer, IDC_OPTION_EDIT_LABEL + i);
        if (hw) SetWindowPos(hw, NULL, 68, by + 17, cx - 92, 22, SWP_NOZORDER);

        hw = GetDlgItem(g.hScrollContainer, IDC_OPTION_EDIT_CMD + i);
        if (hw) SetWindowPos(hw, NULL, 76, by + 43, cx - 100, 22, SWP_NOZORDER);

        hw = GetDlgItem(g.hScrollContainer, IDC_OPTION_COMBO_MODE + i);
        if (hw) SetWindowPos(hw, NULL, 60, by + 69, cx - 84, 120, SWP_NOZORDER);
    }
}

// Update scroll bar info for the container
...
// ---------------------------------------------------------------------------
// Scroll Container Window Procedure (custom class)
// ---------------------------------------------------------------------------
static LRESULT CALLBACK ScrollProc(HWND h,UINT m,WPARAM w,LPARAM l) {
    ...
    case WM_ERASEBKGND:
        ...
        return 1; // Light mode: return 1 to indicate background was erased
    case WM_PAINT: {
        PAINTSTRUCT ps; HDC dc=BeginPaint(h,&ps);
        RECT rc; GetClientRect(h,&rc);
        HBRUSH br = g.hbrBg ? g.hbrBg : (HBRUSH)GetStockObject(WHITE_BRUSH);
        FillRect(dc,&rc,br);
        EndPaint(h,&ps);
        return 0;
    }
...
static LRESULT CALLBACK WndProc(HWND h,UINT m,WPARAM w,LPARAM l) {
    ...
    case WM_CREATE: {
        ...
        // Create scroll container using custom registered class
        g.hScrollContainer=CreateWindowExW(0,SCROLL_CLASS_NAME,NULL,
    ...
    case WM_SIZE: {
        ...
        // Reposition theme button (top-right corner)
        SetWindowPos(g.hwndThemeToggle,NULL,max(760,W-50),8,30,24,SWP_NOZORDER);
        // Reposition title edit field
        SetWindowPos(g.hEditTitle,NULL,102,9,min(300,W-200),24,SWP_NOZORDER);
        // UpDown buddy stays at same position
        SetWindowPos(g.hUpDown,NULL,162,39,0,0,SWP_NOZORDER|SWP_NOSIZE);

        // Bottom section: preview (~160px) + generate button (32px) + padding
        int bottomMargin = 8;
        int previewH = 160;
        int generateBtnH = 32;
        int bottomSectionH = previewH + generateBtnH + 10 + bottomMargin;

        // Y position where the bottom section starts
        int bottomSectionY = H - bottomSectionH;
        if (bottomSectionY < 70) bottomSectionY = 70; // prevent collapse

        // Scroll container fills between top controls (y=70) and bottom section
        int scrollY = 70;
        int scrollH = bottomSectionY - scrollY - 4;
        if (scrollH < 50) scrollH = 50;

        // Generate button position
        int generateBtnY = bottomSectionY;
        int previewY = generateBtnY + generateBtnH + 8;

        SetWindowPos(g.hScrollContainer,NULL,0,scrollY,W,scrollH,SWP_NOZORDER);
        SetWindowPos(g.hBtnGenerate,NULL,10,generateBtnY,120,32,SWP_NOZORDER);
        SetWindowPos(g.hPreview,NULL,10,previewY,max(1,W-20),max(1,previewH),SWP_NOZORDER);

        // Notify scroll container of its new size
        if (g.hScrollContainer) {
            RECT rc;
            GetClientRect(g.hScrollContainer,&rc);
...
int WINAPI WinMain(HINSTANCE hI, HINSTANCE, LPSTR, int nC) {
    ...
    // Register the scroll container custom window class
    WNDCLASSEXW sc = {};
    sc.cbSize = sizeof(WNDCLASSEXW);
    sc.lpfnWndProc = ScrollProc;
    sc.hInstance = hI;
    sc.hCursor = LoadCursorW(NULL, IDC_ARROW);
    sc.hbrBackground = NULL; // We handle painting ourselves
    sc.lpszClassName = SCROLL_CLASS_NAME;
    RegisterClassExW(&sc);
    ...
    int W = 900, H = 700, sW = GetSystemMetrics(SM_CXSCREEN), sH = GetSystemMetrics(SM_CYSCREEN);
